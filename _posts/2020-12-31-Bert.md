...
即run_pretraining.py采用NSP+MASK的loss. 而run_clasifier.py应该采用的是交叉熵loss.
```
def model_fn(features, labels, mode, params):  
  input_ids = features["input_ids"]
  input_mask = features["input_mask"]
...
### !!!get_masked_lm_output函数用于计算语言模型的Loss!!!
...
## Bert Mask机制的修改以适应相应的任务
Bert原有的MASK机制： 对所有正确的句子8,1,1

1. 拼写纠错
增加对错误句子的处理，修改映射关系，首先对错误句子使用 probability概率进行Mask, 然后将其中错误字100% Mask 然后映射到对应的正确字符。这样建立了错误字和正确字的映射关系。具体做法是for i,token in enumerate 正确句子，if token!= wrong_token[i] 则100%mask,else 将token按概率文件mask.

去掉NSP的影响，是否直接将Loss的计算去掉即可？那为什么还要构建 token b?


## Bert finetune后模型变大解决方案

[Bert finetune后模型变大解决方案](https://blog.csdn.net/ljp1919/article/details/100652794)


## 让拼音也能成为token 而不进行piece split 110行 split_tokens.append(token)

next_predict_batch， 将字 映射 为 id
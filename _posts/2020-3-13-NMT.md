...
with open(path+name, "r", encoding="utf-8") as f:
    data = f.read().splitlines() # 读取所有，字符串形式
    ...
    # 对句子中的标点符号进行处理
    # 1. 去掉一个句子的 句号，即 . split 已处理; 将 , 与字分开
    # 2. 将引号分开，所有格形式直接还原， replace("'s",'').replace("'"," ' ")
    # 3. 将连字符号 - 切开 replace('-',' ')
    with open('src_en.txt','w',encoding='utf-8') as f_out:
        for sentence in sentences:
            if not sentence:
                continue
            sentence = sentence.replace(",",' , ').replace("'s",' ').replace("'"," ' ").replace('-',' ').replace('"',' " ').replace('!',' ! ').replace(':',' : ')
            f_out.write(sentence.lstrip()+'\n')
